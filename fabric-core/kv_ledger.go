...
func typedDeltaCapabilityEnabled(blockStore *blkstorage.BlockStore, genesisBlock *common.Block) bool {
	bcInfo, err := blockStore.GetBlockchainInfo()
	if err != nil {
		logger.Warnf("typed-delta capability: cannot read blockchain info (%v); defaulting to disabled", err)
		return false
	}

	if bcInfo.Height == 0 {
		if genesisBlock == nil {
			logger.Warnf("typed-delta capability: empty block store and no genesis block available; defaulting to disabled")
			return false
		}
		enabled, err := channelconfig.ExtractTypedDeltaCapability(genesisBlock, factory.GetDefault())
		if err != nil {
			logger.Warnf("typed-delta capability: cannot read from genesis block (%v); defaulting to disabled", err)
			return false
		}
		return enabled
	}

	latestBlock, err := blockStore.RetrieveBlockByNumber(bcInfo.Height - 1)
	if err != nil {
		logger.Warnf("typed-delta capability: cannot retrieve latest block (%v); defaulting to disabled", err)
		return false
	}
	cfgIndex, err := protoutil.GetLastConfigIndexFromBlock(latestBlock)
	if err != nil {
		logger.Warnf("typed-delta capability: cannot read last-config index (%v); defaulting to disabled", err)
		return false
	}
	cfgBlock, err := blockStore.RetrieveBlockByNumber(cfgIndex)
	if err != nil {
		logger.Warnf("typed-delta capability: cannot retrieve config block %d (%v); defaulting to disabled", cfgIndex, err)
		return false
	}
	enabled, err := channelconfig.ExtractTypedDeltaCapability(cfgBlock, factory.GetDefault())
	if err != nil {
		logger.Warnf("typed-delta capability: cannot parse channel config (%v); defaulting to disabled", err)
		return false
	}
	return enabled
}
...
func newKVLedger(initializer *lgrInitializer) (*kvLedger, error) {
	...
	txmgrInitializer := &txmgr.Initializer{
		...
		TypedDeltaValidation: typedDeltaCapabilityEnabled(initializer.blockStore, initializer.genesisBlock),
	}
