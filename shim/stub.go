...
func (s *ChaincodeStub) PutState(key string, value []byte) error {
	...
	if s.deltaKeys[key] {
		return errors.New("a plain write and a typed delta on the same key in one transaction are not allowed")
	}
	if s.plainKeys == nil {
		s.plainKeys = map[string]bool{}
	}
...
type DeltaOp = kvrwset.DeltaDescriptor_Op

const (
	DeltaAdd = kvrwset.DeltaDescriptor_ADD
	DeltaSub = kvrwset.DeltaDescriptor_SUB
)

func buildShimDelta(typeID string, op DeltaOp, arg []byte, schemaVersion []byte) (*kvrwset.DeltaDescriptor, error) {
	if op != DeltaAdd && op != DeltaSub {
		return nil, errors.New("unsupported delta op")
	}
	if len(arg) != 8 {
		return nil, fmt.Errorf("delta arg must be a canonical 8-byte int64, got %d bytes", len(arg))
	}
	if int64(binary.BigEndian.Uint64(arg)) < 0 {
		return nil, errors.New("delta magnitude must be non-negative (direction is carried by the op)")
	}
	argCopy := append([]byte(nil), arg...)
	var svCopy []byte
	if len(schemaVersion) > 0 {
		svCopy = append([]byte(nil), schemaVersion...)
	}
	return &kvrwset.DeltaDescriptor{TypeId: typeID, Op: op, Arg: argCopy, SchemaVersion: svCopy}, nil
}

func (s *ChaincodeStub) PutDelta(key, typeID string, op DeltaOp, arg []byte, schemaVersion []byte) error {
	if key == "" {
		return errors.New("key must not be an empty string")
	}
	if s.didRead {
		return errors.New("PutDelta after a state read: the delta arg must derive from proposal inputs only")
	}
	if s.plainKeys[key] {
		return errors.New("a plain write and a typed delta on the same key in one transaction are not allowed")
	}
	delta, err := buildShimDelta(typeID, op, arg, schemaVersion)
	if err != nil {
		return err
	}
	if s.deltaKeys == nil {
		s.deltaKeys = map[string]bool{}
	}
	s.deltaKeys[key] = true
	return s.handler.handlePutDelta(key, delta, s.ChannelID, s.TxID)
}

func PutDelta(stub ChaincodeStubInterface, key, typeID string, op DeltaOp, arg []byte, schemaVersion []byte) error {
	cs, ok := stub.(*ChaincodeStub)
	if !ok {
		return errors.New("PutDelta requires *ChaincodeStub; mock/interface path unsupported")
	}
	return cs.PutDelta(key, typeID, op, arg, schemaVersion)
}
...
func (s *ChaincodeStub) DelState(key string) error {
	if s.deltaKeys[key] {
		return errors.New("a plain write/delete and a typed delta on the same key in one transaction are not allowed")
	}
	if s.plainKeys == nil {
		s.plainKeys = map[string]bool{}
	}
