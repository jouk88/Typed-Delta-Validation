...
func testutilCheckKeyNotInRange(t *testing.T, hqe ledger.HistoryQueryExecutor, ns, desiredKey, falseKey string) {
	q := hqe.(*QueryExecutor)
	rangeScanDesired := constructRangeScan(ns, desiredKey)
	dbItr, err := q.levelDB.GetIterator(rangeScanDesired.startKey, rangeScanDesired.endKey)
	require.NoError(t, err, "Error upon GetIterator()")
	defer dbItr.Release()
